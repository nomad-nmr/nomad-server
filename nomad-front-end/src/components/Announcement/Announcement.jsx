...
const Announcement = ({ announcement }) => {
  ...
  const kindConfig = {
    warning: {
      ...
      style: { border: '1px solid #ff4d4f', backgroundColor: '#fff2f0', color: '#a8071a' }
    },
    info: {
      icon: <InfoCircleOutlined style={{ color: '#1677ff' }} />,
      style: { border: '1px solid #1677ff', backgroundColor: '#e6f4ff', color: '#0958d9' }
    },
    news: {
      icon: <NotificationOutlined style={{ color: '#52c41a' }} />,
      style: { border: '1px solid #52c41a', backgroundColor: '#f6ffed', color: '#389e0d' }
    }
  }
  ...
  return (
    <div style={{ marginBottom: '1.5rem' }}>
      <Alert
        title={<strong>{announcement.title || 'Announcement'}</strong>}
        description={
          <div
            className='ql-editor'
